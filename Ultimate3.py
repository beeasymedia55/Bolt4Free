import os
import sys
import re
import time
import random
import threading
import requests
from urllib.parse import urlencode
from colorama import init, Fore, Style

# --- Initialisierung ---
init(autoreset=True)
_lock = threading.Lock()
success = 0
errors = 0

class TikTokSigner:
    @staticmethod
    def get_headers(did, iid):
        # Basis-Header für die Kommunikation
        return {
            "User-Agent": "com.ss.android.ugc.trill/2613 (Linux; U; Android 12; en_US; Pixel 6 Pro)",
            "Accept-Encoding": "gzip",
            "Connection": "keep-alive",
            "x-common-params-v2": "version_code=26.1.3&app_name=musically_go&device_platform=android",
            "x-did": did,
            "x-iid": iid
        }

def load_list(filename):
    try:
        with open(filename, "r", encoding="utf-8") as f:
            return [line.strip() for line in f if line.strip()]
    except FileNotFoundError:
        return []

# --- Room ID Grabber (aus Ultimate.py) ---
def grab_and_save_room_id(username):
    print(f"{Fore.YELLOW}[*] Suche Room ID für @{username}...")
    try:
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Safari/537.36",
            "Referer": "https://www.tiktok.com/"
        }
        response = requests.get(f"https://www.tiktok.com/@{username}/live", headers=headers, timeout=15)
        room_id = re.search(r'room_id=(\d+)', response.text)
        if not room_id:
            room_id = re.search(r'"roomId":"(\d+)"', response.text)
            
        if room_id:
            final_id = room_id.group(1)
            with open("room_id.txt", "w") as f:
                f.write(final_id)
            print(f"{Fore.GREEN}[+] Room ID gefunden: {final_id}")
            return final_id
        else:
            print(f"{Fore.RED}[!] Keine Room ID gefunden. Ist der User LIVE?")
            return None
    except Exception as e:
        print(f"{Fore.RED}[!] Fehler: {e}")
        return None

# --- Live Interaktions-Funktionen ---

def sendLiveEnter(did, iid, room_id):
    """Logik für Room Enter"""
    global success, errors
    try:
        params = {
            "room_id": room_id,
            "device_id": did,
            "iid": iid,
            "version_code": "26.1.3",
            "device_platform": "android",
            "aid": "1988" # aid aus Share-Live.py übernommen
        }
        url = "https://webcast16-normal-c-useast1a.tiktokv.com/webcast/room/enter/?" + urlencode(params)
        resp = requests.post(url, headers=TikTokSigner.get_headers(did, iid), timeout=10)
        if resp.status_code == 200:
            with _lock:
                success += 1
                print(f"{Fore.CYAN}[ENTER] Success | Total: {success}")
    except:
        with _lock: errors += 1

def sendLiveLike(did, iid, room_id):
    """Logik für Likes"""
    global success, errors
    try:
        params = {
            "room_id": room_id,
            "device_id": did,
            "iid": iid,
            "version_code": "26.1.3",
            "count": random.randint(1, 3)
        }
        url = "https://webcast16-normal-c-useast1a.tiktokv.com/webcast/room/like/?" + urlencode(params)
        resp = requests.post(url, headers=TikTokSigner.get_headers(did, iid), timeout=10)
        if resp.status_code == 200:
            with _lock:
                success += 1
                print(f"{Fore.LIGHTRED_EX}[LIKE] Liked! | Total: {success}")
    except:
        with _lock: errors += 1

def sendLiveShare(did, iid, room_id):
    """EXAKTE Logik und Endpoint aus Share-Live.py"""
    global success, errors
    try:
        # Parameter exakt wie in Share-Live.py
        appName = random.choice(["tiktok_web", "musically_go"])
        platform = random.choice(["android", "windows", "iphone"])
        
        url = (f"https://webcast.tiktok.com/webcast/room/share/?"
               f"aid=1988&app_language=ar&app_name={appName}&"
               f"browser_language=ar&browser_platform={platform}&"
               f"cookie_enabled=true&device_id={did}&"
               f"device_platform={platform}&room_id={room_id}&share_type=2")
        
        # Header Logik aus Share-Live.py
        headers = TikTokSigner.get_headers(did, iid)
        headers["Content-Type"] = "application/x-www-form-urlencoded; charset=UTF-8"
        
        resp = requests.post(url, headers=headers, timeout=10)
        
        # Erfolgskontrolle wie im Original
        if '"status_code":0' in resp.text or resp.status_code == 200:
            with _lock:
                success += 1
                print(f"{Fore.BLUE}[SHARE] Shared! | Total: {success}")
        else:
            with _lock: errors += 1
    except:
        with _lock: errors += 1

# --- Hauptmenü ---

def Main():
    global success
    os.system('cls' if os.name == 'nt' else 'clear')
    print(f"{Fore.MAGENTA}TikTok Live Ultimate (Integrated Share Logic)")
    print("---------------------------------------")
    print("0. GRAB Room ID from Username")
    print("1. Live Room Enter")
    print("2. Live Room Like")
    print("3. Live Room Share (Share-Live.py Logic)")
    print("---------------------------------------")
    
    choice = input("Choice: ")

    if choice == "0":
        user = input("Enter Username (ohne @): ")
        grab_and_save_room_id(user)
        input("\nEnter drücken...")
        return Main()

    threads_count = int(input("Threads: "))
    amount_target = int(input("Target Amount: "))
    
    devices = load_list("devices.txt")
    room_ids = load_list("room_id.txt")
    
    if not devices or not room_ids:
        print(f"{Fore.RED}Fehler: devices.txt oder room_id.txt leer!")
        return

    print(f"\n{Fore.WHITE}Angriff gestartet auf Room: {room_ids[0]}...\n")

    while success < amount_target:
        if threading.active_count() <= threads_count:
            device = random.choice(devices)
            try:
                # Nutzt das did:iid Format aus deiner devices.txt
                parts = device.split(':')
                did, iid = parts[0], parts[1]
                r_id = room_ids[0]

                if choice == "1":
                    threading.Thread(target=sendLiveEnter, args=(did, iid, r_id)).start()
                elif choice == "2":
                    threading.Thread(target=sendLiveLike, args=(did, iid, r_id)).start()
                elif choice == "3":
                    threading.Thread(target=sendLiveShare, args=(did, iid, r_id)).start()
            except:
                continue
            
            time.sleep(0.01)

if __name__ == "__main__":
    Main()
