...
def set_title(title):
    if os.name == 'nt':
        ...
        # Für Termux/Linux Terminal Titel
...
# --- CORE BOT FUNCTIONS ---
def sendHearts(did, iid, cdid, udid, room_id, proxies=None):
    ...
    try:
        ...
        params = urlencode({
            ...
            "cdid": cdid, "openudid": udid, "aid": "1988",
...
def sendViews(did, iid, cdid, udid, room_id, proxies=None):
    global success, fails
    try:
        params = urlencode({
            "room_id": room_id, "device_id": did, "install_id": iid,
...
if __name__ == "__main__":
    ...
    if not os.path.exists("devices.txt"):
        print(f"{Fore.RED}devices.txt fehlt!"); exit()
    
    devices = open("devices.txt").read().splitlines()
    proxies = open("proxies.txt").read().splitlines() if os.path.exists("proxies.txt") else []
    ...
    def run():
        while True:
            try:
                ...
                did, iid, cdid, udid = parts[0], parts[1], parts[2], parts[3]

                if choice_type == "1":
                    sendViews(did, iid, cdid, udid, room_id, proxies)
                else:
                    sendHearts(did, iid, cdid, udid, room_id, proxies)
                
                # Print Status
                sys.stdout.write(f"\r[{Fore.GREEN}HIT{Fore.RESET}] Success: {success} | Fails: {fails}")
                sys.stdout.flush()
    ...
    while True:
        set_title(f"Success: {success} | Fails: {fails}")
        time.sleep(1)
