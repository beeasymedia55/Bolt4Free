...
# =============================================================================
# PROXY TESTER - With Thread Input & Save Option
# =============================================================================
class ProxyTester:
    @staticmethod
    ...
    def get_test_threads():
        """Ask how many threads to use for testing"""
        print("\n🧵 Testing Threads:")
        ...
        if choice == '':
            return 50
        try:
            return max(1, min(500, int(choice)))
        except:
            return 50
...
║  ✓ Thread Input for Proxy Testing                           ║
        ...
        if choice == 'Q':
            ...
            while True:
                ...
                if pc == '0': break
                    ...
                    if proxies:
                        save = input(f" Save all to file? (y/n): ").strip().lower()
                        if save == 'y':
                            filename = input(" Filename (default: proxies_scraped.txt): ").strip() or "proxies_scraped.txt"
                            with open(filename, 'w') as f:
                                for p in proxies:
                                    f.write(p + '\n')
                            print(f" ✅ Saved {len(proxies)} to {filename}")
