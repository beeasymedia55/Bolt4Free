...
"""
═══════════════════════════════════════════════════════════════════════════════
  TikTok Live Viewer Bot  v5.1  —  Pure HTTP/WebSocket + REAL Signatures
═══════════════════════════════════════════════════════════════════════════════
  Signing pipeline:

    [1] X-Bogus          →  TikTok JS VM (X-Bogus.js) executed via Node/execjs
    [2] _signature       →  external signer hook (env TIKTOK_SIGNER_URL)
  ...
  Connection flow:
    sign() → /api-live/user/room/ → room_id → /webcast/room/info/
          → push_server + cursor → WSS://{push_server}/ws/ → protobuf ack+ping

  v5.1 FIXES:
    * 'NoneType' object has no attribute 'get' — viewers were spawned with
      room=None. spawn()/set_target()/auto_scale() now resolve the room first
      and refuse cleanly when no target is configured.
    * auto_scale capped at CFG.max_viewers.
    * getpeername() guarded against dead sockets after handshake.
═══════════════════════════════════════════════════════════════════════════════
"""
...
# ────────────────────────────────────────────────────────────────────────────
# 4. SIGNER SERVICE  —  the `sign()` function
# ────────────────────────────────────────────────────────────────────────────
...
class WebcastClient(threading.Thread):
    ...
    def _ws_url(self) -> str:
        # FIX: guard against room=None — raise a descriptive error instead of
        #      'NoneType' object has no attribute 'get'
        if self.room is None:
            raise RuntimeError(
                "room is None — resolve the room first (menu option 1) "
                "before spawning viewers"
            )
    ...
    def _connect_once(self):
        ...
        # FIX: getpeername() can throw if the socket died right after handshake
        try:
...
class ViewerOrchestrator:
    ...
    def resolve_room(self) -> bool:
        # FIX: refuse cleanly if no target is configured at all
        if not CFG.target_username and not CFG.room_id:
            log.error("[orchestrator] no target configured — set a username (settings → 8) "
                      "or room_id (settings → 9) first")
            return False
    ...
    def spawn(self, count: int = 1):
        # FIX: auto-resolve room before spawning — never hand None to clients
        if self.room is None:
    ...
    def set_target(self, target: int):
        # FIX: same guard — resolve room before adding viewers
        if self.room is None:
            log.warning("[orchestrator] room not resolved yet — resolving now ...")
            if not self.resolve_room():
    ...
    def auto_scale(self, target: int, step: int = None, interval: float = None):
        # FIX: cap at max_viewers and resolve room first
...
def menu_auto_scale(orchestrator: ViewerOrchestrator):
    # FIX: resolve room before asking for the target
    if not orchestrator.room and not orchestrator.resolve_room():
        print("  cannot auto-scale — resolve a room first (option 1 or set username in settings)")
        return
    try:
...
BANNER = r"""
 ...
 ╚══██╔══╝██║ ██╔╝██╔════╝████╗  ██║   ██║██║██║   ██║██╔════╝
    ...
    ██║   ██║  ██╗███████╗██║ ╚████║   ██║██║ ╚████╔╝ ███████╗
    ╚═╝   ╚═╝  ╚═╝╚══════╝╚═╝  ╚═══╝   ╚═╝╚═╝  ╚═══╝  ╚══════╝
        LIVE VIEWER BOT v5.1 — REAL X-Bogus / X-Gorgon / X-Argus
"""
