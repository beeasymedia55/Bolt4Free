...
SignerPy import at top • Webcast proxy testing • All live actions
"""

# =============================================================================
# SignerPy IMPORT - MUST BE AT THE VERY TOP
# =============================================================================
import signerpy
from signerpy import XBogusSigner, GorgonSigner, Signer

# Verify SignerPy is loaded
try:
    SIGNERPY_VERSION = signerpy.__version__
    print(f"✅ SignerPy v{SIGNERPY_VERSION} loaded successfully")
except AttributeError:
    SIGNERPY_VERSION = "unknown"
    print(f"✅ SignerPy loaded (version unknown)")

SIGNERPY_AVAILABLE = True

# =============================================================================
# STANDARD LIBRARY IMPORTS (AFTER SignerPy)
# =============================================================================
import os
import sys
import ssl
...
# =============================================================================
# SignerPy WRAPPER - Production Signature Generation
# =============================================================================

class SignerPyWrapper:
    """
    Wrapper around SignerPy for TikTok header generation
    Uses XBogusSigner, GorgonSigner from SignerPy library
    SignerPy is ALREADY imported at the top of this file
    """
    ...
    def _init_signers(self):
        """Initialize SignerPy components"""
        try:
            ...
            print("✅ SignerPy components initialized")
        except Exception as e:
            print(f"⚠️  SignerPy init error: {e}")
            self.signer_initialized = False
    
    def generate_xbogus(self, params_dict, user_agent="", cookies=""):
        """
        Generate X-Bogus using SignerPy
    ...
    def generate_gorgon(self, params_dict, cookies=None, data=None, unix=None):
        """Generate X-Gorgon header using SignerPy"""
    ...
    def generate_full_signature(self, params_dict, headers_dict=None):
        """
        Generate COMPLETE signature set using SignerPy
    ...
    def _fallback_xbogus(self, params_dict, user_agent=""):
        """Built-in X-Bogus generation when SignerPy fails"""
        params_str = urlencode(sorted(params_dict.items()))
...
# =============================================================================
# MEDO-STYLE HEADER GENERATOR (with SignerPy)
# =============================================================================

class MedoHeaderGenerator:
    """
    Medo/SignerPy style header generator for TikTok
    Uses SignerPy for ALL signature generation
    """
    
    def __init__(self):
        self.signer = SignerPyWrapper()
    ...
    def generate_headers(self, device_profile=None, extra_params=None):
        """
        Generate COMPLETE TikTok headers using SignerPy signatures
        ...
        # Use SignerPy to generate signatures
        if extra_params:
    ...
    def get_signer_status(self):
        return {
            ...
            'using_signerpy': SIGNERPY_AVAILABLE and self.signer.signer_initialized,
            'signerpy_version': SIGNERPY_VERSION
        }
...
class TikTokLiveWorker:
    """Worker that interacts with TikTok Live using SignerPy signatures"""
    ...
    def _make_request(self, endpoint, params, payload=None, method='POST'):
        """Make signed request to TikTok using SignerPy"""
...
class TikTokLiveBot:
    ...
    def start(self):
        ...
        print(f"   SignerPy v{sig_status['signerpy_version']}: {'✅ ACTIVE' if sig_status['using_signerpy'] else '❌ FALLBACK'}")
...
╚══════════════════════════════════════════════════════════════╝
    ...
    # Show SignerPy status
    sig_status = bot.header_gen.get_signer_status()
    if sig_status['using_signerpy']:
        print(f"🔐 SignerPy v{sig_status['signerpy_version']}: ✅ ACTIVE - Production signatures\n")
    else:
        print("🔐 SignerPy: ❌ ERROR - Using fallback signatures\n")
    ...
    while True:
        ...
        if choice == 'Q':
            ...
            clear_screen()
            print("\n🔐 SignerPy STATUS:")
            print(f"   Library available:    {'✅ YES' if sig_status['signerpy_available'] else '❌ NO'}")
            print(f"   Signer initialized:   {'✅ YES' if sig_status['signer_initialized'] else '❌ NO'}")
            print(f"   Using SignerPy:       {'✅ ACTIVE' if sig_status['using_signerpy'] else '❌ FALLBACK'}")
            print(f"   SignerPy version:     {sig_status['signerpy_version']}")
            print("\n   ✅ Production signatures ACTIVE")
            print("   - X-Bogus: Generated by SignerPy")
            print("   - X-Gorgon: Generated by SignerPy")
