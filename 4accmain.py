...
def send_tiktok_code(session, email, params, install_id, password):
    ...
    headers = {
        ...
        'X-SS-STUB': m['x-ss-stub'], 'X-Ladon': m['x-ladon'],
        'X-Khronos': m['x-khronos'], 'X-Argus': m['x-argus'], 'X-Gorgon': m['x-gorgon'],
    }
    resp = session.post(url, params=params, data=payload, headers=headers)
    
    # FIX: Check if response is valid JSON to prevent crash
    try:
        return resp.json()
    except requests.exceptions.JSONDecodeError:
        print(f"[-] TikTok returned invalid response (IP likely blocked). Status: {resp.status_code}")
        return None
    ...
    while True:
        try:
            ...
            if not resp_send or resp_send.get("message") == "error":
                print("[-] TikTok blocked the request. Retrying in 5s...")
                time.sleep(5)
                continue
            ...
            try:
                ...
                if session_key:
                    ...
                    print(f"[+] Success! Bot saved: {username}")
                else:
                    print(f"[-] Registration failed: {resp_verify.get('message')}")
            except:
