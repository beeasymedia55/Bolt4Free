...
# Default devices (auto-generated if not from file)
DEFAULT_DEVICES = [
...
# =============================================================================
# PROXY TESTER - With Custom URL Option
# =============================================================================
class ProxyTester:
    @staticmethod
    def test_proxy_standard(proxy_str, test_url="http://httpbin.org/ip"):
        """Standard proxy test against any URL"""
    ...
    def test_proxy_webcast(proxy_str):
        """Test proxy against TikTok webcast"""
        proxy_url = f"http://{proxy_str}"
    ...
    def test_batch(proxies, test_method='1', proxy_type='http', custom_url=None, max_workers=50):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for p in test_proxies:
                if proxy_type != 'http':
                    proxy_url = f"{proxy_type}://{p}"
                else:
                    proxy_url = f"http://{p}"
                
                if test_method == '1':
                    futures[executor.submit(ProxyTester.test_proxy_standard, p)] = (p, proxy_url)
                elif test_method == '2':
                    futures[executor.submit(ProxyTester.test_proxy_webcast, p)] = (p, proxy_url)
                elif test_method == '3' and custom_url:
                    futures[executor.submit(ProxyTester.test_proxy_custom, p, custom_url)] = (p, proxy_url)
                elif test_method == '4':
                    # Test both standard AND webcast
                    futures[executor.submit(ProxyTester.test_proxy_standard, p)] = (p, proxy_url, 'standard')
                    futures[executor.submit(ProxyTester.test_proxy_webcast, p)] = (p, proxy_url, 'webcast')
                else:
                    futures[executor.submit(ProxyTester.test_proxy_standard, p)] = (p, proxy_url)
            ...
            for future in as_completed(futures):
                ...
                if test_method == '4':
                    # For "all methods", need both to pass
                    test_type = result[2] if len(result) > 2 else 'unknown'
                ...
                if completed % 25 == 0 or completed == len(test_proxies):
...
class TikTokLiveBot:
    ...
    def start(self):
        ...
        print(f"   Devices: {get_device_count()} (from file: {self.use_devices_from_file})")
...
╚══════════════════════════════════════════════════════════════╝
        ...
        if choice == '0': break
            ...
            for i, d in enumerate(all_devs[:20]):  # Show first 20
